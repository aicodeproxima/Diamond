...
/**
 * Neural synapse — drifting neurons, occasional lightning bolts between
 * pairs that come close. Each bolt is a zigzag path that fades quickly.
 * Cursor sparks extra flashes in its vicinity.
 *
 * @param {object} [props]
 * @param {number} [props.nodeCount=70]
 * @param {number} [props.firingRange=120]   Max distance to trigger a flash.
 * @param {number} [props.fireChance=0.004]  Per-pair per-frame flash chance.
 * @param {number} [props.hue=195]
    ...
    const frame = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (d < firingRange) {
            // higher fire chance when close OR when cursor near either endpoint
