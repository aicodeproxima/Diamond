...
/**
 * Voronoi / cellular — seed points drift and colored cells fill the canvas.
 * Uses a coarse grid sampling (every N pixels) for performance instead of a
 * true Fortune's algorithm. Cursor adds a temporary seed that repels cells.
 *
 * @param {object} [props]
 * @param {number} [props.seedCount=28]
 * @param {number} [props.gridStep=6]   Smaller = crisper edges, more CPU.
 * @param {number[]} [props.hueRange=[200,320]]
...
export default function VoronoiBackground({
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      // coarse-sampled voronoi: for each grid cell, find nearest seed
      const step = gridStep
      for (let y = 0; y < h; y += step) {
        for (let x = 0; x < w; x += step) {
          ...
          const s = seeds[best]
          // edge darkening: find 2nd nearest, darken if close to boundary
          let bestD2 = Infinity
          for (let i = 0; i < seeds.length; i++) {
            if (i === best) continue
            const sO = seeds[i]
      ...
      // draw seed dots for a little glow
