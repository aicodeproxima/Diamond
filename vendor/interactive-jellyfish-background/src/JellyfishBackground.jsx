...
/**
 * Jellyfish tendrils — chains of linked points via verlet integration.
 * Each jelly drifts with a sine-wave bob; its tentacles trail behind
 * with springy physics. Cursor attracts jellies toward it.
 *
 * @param {object} [props]
 * @param {number} [props.jellyCount=6]
 * @param {number} [props.tendrilsPerJelly=6]
 * @param {number} [props.segmentsPerTendril=18]
 * @param {number[]} [props.hueRange=[170,260]]
    ...
    const build = () => {
      ...
      for (let i = 0; i < jellyCount; i++) {
        ...
        jellies.push({
          ...
          bobSpeed: 0.001 + Math.random() * 0.0015,
          tendrils,
        })
      }
    }
      ...
      for (const j of jellies) {
        ...
        for (let ti = 0; ti < j.tendrils.length; ti++) {
          ...
          for (let k = 1; k < ten.segments.length; k++) {
            ...
            s.py = s.y
            s.x += vx
            s.y += vy + 0.14 + Math.sin(t * 0.002 + ti + k * 0.35) * 0.08
          }
          // constraint pass — keep equal spacing
