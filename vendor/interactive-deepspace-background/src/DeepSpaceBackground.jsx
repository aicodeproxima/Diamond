...
/**
 * Deep space — 600 twinkling stars, multiple soft galaxies drifting in
 * the distance, and 20+ shooting stars continuously streaking across in
 * many different directions on loop.
 *
 * @param {object} [props]
 * @param {number} [props.starCount=600]
 * @param {number} [props.galaxyCount=4]
 * @param {number} [props.shootingStarCount=22]
 */
export default function DeepSpaceBackground({
  ...
  useEffect(() => {
    ...
    // seeded directions so all 20+ shoot in roughly-unique angles
    const dirs = []
    const buildDirections = () => {
      const count = Math.max(20, shootingStarCount)
      dirs.length = 0
      for (let i = 0; i < count; i++) {
        // spread across full 360° with a little jitter so trajectories vary
    ...
    const spawnShooter = (i) => {
      ...
      // Start off-screen on the side OPPOSITE the travel direction
      const margin = 80
      let x, y
      if (cos > 0) x = -margin
      else x = w + margin
      ...
      // mix: offset along the perpendicular
    ...
    const drawGalaxy = (g, t) => {
      ...
      // spiral arms — draw via rotated/scaled ellipses with gradient
      ctx.save()
    ...
    const frame = (t) => {
      ...
      // deep-space background with subtle radial nebula wash
      ctx.fillStyle = '#02010a'
      ctx.fillRect(0, 0, w, h)

      ctx.globalCompositeOperation = 'lighter'

      // galaxies first (background layer)
      ...
      for (let i = 0; i < shootingStars.length; i++) {
        ...
        if (ss.delay > 0) { ss.delay--; continue }
        ss.x += ss.vx
        ss.y += ss.vy
        // fade tail — draw a gradient line
