...
/**
 * Fluid / smoke — simplified stable-fluids-style grid advection.
 * Runs on a coarse grid (80x45) for performance; upscaled with bilinear
 * when rendered. Cursor injects color + velocity into the fluid.
 *
 * Real Navier–Stokes is heavy; this uses a lightweight advection +
 * diffusion pass that looks 80% as good.
 *
 * @param {object} [props]
 * @param {number} [props.gridCols=80]
 * @param {number} [props.gridRows=45]
 * @param {number[]} [props.hueRange=[180,320]]
 * @param {boolean} [props.interactive=true]
 */
export default function FluidSmokeBackground({
  fixed = true,
  zIndex = 0,
  className = '',
  style = {},
  ...
  useEffect(() => {
    ...
    const step = (t) => {
      ...
      advect(hueField, density0, vx, vy, dt)  // reuse buffer for hue advect
      // swap
      let tmp = density; density = density0; density0 = tmp
      tmp = vx; vx = vx0; vx0 = tmp
      tmp = vy; vy = vy0; vy0 = tmp
      // hueField we keep as new copy from density0 — approximate
      // dissipate
